...
from ..models.database import Story, Keyword
...
class StoryGenerator:
    ...
    async def extract_keywords(self, story: StoryResponse) -> List[str]:
        prompt = ChatPromptTemplate.from_messages([
            ("system", """You are a keyword extractor for RPG stories. Extract the most important keywords that would be useful for searching and categorization.
            ...
            ("user", f"""Extract keywords from this story:
    ...
    async def save_to_database(self, story: StoryResponse, request: StoryRequest, keywords: List[str], db: Session) -> Story:
        # Create story record
        db_story = Story(
            title=story.title,
            summary=story.summary,
    ...
    async def generate_story(self, request: StoryRequest, db: Session) -> Story:
        prompt = ChatPromptTemplate.from_messages([
            ("system", """You are an expert RPG story generator. Create an engaging one-shot story
            ...
            ("user", """Generate a {length} {rpg_system} one-shot story with the following parameters:
            - Theme: {theme}
        ...
        custom_theme_text = f"- Custom Theme: {request.custom_theme}" if request.custom_theme else ""
